...
class PosePipeline:
    ...
    def __init__(self, T_base_link, T_link_camera, joint_axis: str = "z"):
        # 입력 변환을 float 타입 numpy 배열로 변환
        T_bl = np.asarray(T_base_link, dtype=float)
        T_lc = np.asarray(T_link_camera, dtype=float)

        # shape 검증: (4, 4)가 아니면 ValueError
        if T_bl.shape != (4, 4) or T_lc.shape != (4, 4):
            raise ValueError(f"변환 행렬은 (4, 4) 크기여야 합니다. (T_base_link: {T_bl.shape}, T_link_camera: {T_lc.shape})")

        if joint_axis not in _ROT:
            raise ValueError(f"지원하지 않는 joint_axis 입니다: {joint_axis!r}. ('x', 'y', 'z' 중 선택)")
    ...
    def T_base_link(self) -> np.ndarray:
        ...
        R = rot_fn(self.joint_angle)
        T_rot = make_T(R, [0.0, 0.0, 0.0])
    ...
    def object_pose_in_base(self, T_camera_object) -> np.ndarray:
        ...
        """
        T_co = np.asarray(T_camera_object, dtype=float)
        return self.T_base_camera @ T_co
