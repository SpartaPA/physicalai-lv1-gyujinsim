...
def test_camera_to_base_matches_chain(pipeline, rng):
    chain = default_chain()
    P_cam = rng.uniform(-1.0, 1.0, size=(100, 3))

    # 1) PosePipeline 결과
    res_pipeline = pipeline.camera_to_base(P_cam)

    # 2) default_chain 결과
    res_chain = chain.transform("base", "camera", P_cam)

    # 3) 직접 행렬 곱 합성 변환 결과
    T_direct = chain.get("base", "link") @ chain.get("link", "camera")
    res_direct = transform_points(T_direct, P_cam)

    assert np.allclose(res_pipeline, res_chain)
    assert np.allclose(res_pipeline, res_direct)
...
def test_roundtrip_restores_points(pipeline, rng):
    # (N, 3) 점군 왕복 검증
    P_cam_multi = rng.uniform(-1.0, 1.0, size=(100, 3))
    P_base_multi = pipeline.camera_to_base(P_cam_multi)
    P_restored_multi = pipeline.base_to_camera(P_base_multi)
    assert np.allclose(P_cam_multi, P_restored_multi)

    # (3,) 단일 점 왕복 검증
    P_cam_single = rng.uniform(-1.0, 1.0, size=(3,))
    P_base_single = pipeline.camera_to_base(P_cam_single)
    P_restored_single = pipeline.base_to_camera(P_base_single)
    assert np.allclose(P_cam_single, P_restored_single)


# --- 권장 추가 테스트 ---------------------------------------------------------

def test_joint_angle_zero_is_nominal(pipeline):
    """set_joint_angle(0) 이면 T_base_link 가 생성자에 준 값 그대로여야 한다."""
    chain = default_chain()
...
def test_joint_angle_changes_result(pipeline, rng):
    """관절 각도를 바꾸면 같은 관측이 base 에서 다른 위치로 가야 한다."""
    ...
    pipeline.set_joint_angle(0.0)  # 원복


def test_rejects_wrong_shape():
    """(3,3) 행렬을 넣으면 ValueError 가 발생해야 한다."""
    wrong_T = np.eye(3)
    valid_T = np.eye(4)
